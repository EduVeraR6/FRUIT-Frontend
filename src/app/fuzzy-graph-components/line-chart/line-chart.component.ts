...
import { Chart, ChartConfiguration, ChartEvent, ChartType, ChartTypeRegistry, LegendElement, LegendItem } from 'chart.js';
import { ChartDataInput, DataPoint } from './lineChart.interface';
    ...
    const config: ChartConfiguration = {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: false,
            text: `Conjunto difuso para la variable lingüística `
          }
        }
      }
    };
  ...
  createChart(data: ChartDataInput, linguisticVariable: string) {
    ...
    data.functions.forEach((func, index) => {
      ...
      if (func.functionType === 'trapezoidal') {
        ...
        console.log(`Función ${index + 1} [${func.linguisticValue}] - Puntos trapezoidales ordenados:`, sorted);
        ...
        console.log(`Función ${index + 1} [${func.linguisticValue}] - Puntos triangulares ordenados:`, sorted);
      }
            ...
            labels: {
              ...
              generateLabels: (chart: Chart) => {
                ...
                return data.datasets.map((dataset, index) => {
                  console.log(this.linguisticVariable);
                  console.log(`Generando etiqueta para el dataset ${index + 1}:`, dataset);
              
                  const combinedLabel = `${this.linguisticVariable} ${dataset.label}`;
              
                  return {
                    text: combinedLabel || `Dataset ${index + 1}`,
